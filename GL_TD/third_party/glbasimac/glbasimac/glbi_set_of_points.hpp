...
struct GLBI_Set_Of_Points {
	...
	// Initialize the set of points to render with A unique color (c_r,c_v,c_b)
	void initSet(const std::vector<float> in_coord,float c_r,float c_v,float c_b);
	// Initialize the set of points to render with a color for each point
	...
	// Allow to switch between points (GL_POINTS) and a line (GL_LINE_STRIP) or any kind of graphique primitive
